...
const closestByClassName = (el: HTMLElement, className: string): HTMLElement | null => {
  ...
  if (document.body.isEqualNode(el)) {
    return null
  }
...
export default class App {
  ...
  private enable = () => {
    ...
    if (classNamesToHide.length) {
      for (const i of classNamesToHide) {
        Array.from(
          this.schema.getElementsByClassName(`st${i}`) as HTMLCollectionOf<HTMLElement>,
  ...
  private mouseDown = (e: MouseEvent) => {
    ...
    if (station) {
      return this.stationClick(station)
    }
  ...
  private wheel = (e: WheelEvent) => {
    e.preventDefault()
    e.deltaY > 0 ? this.tZoomOut() : this.tZoomIn()
  }

  private getSVG = () =>
    this.options.parking ? SVG : SVG.replace(/<path id="park-and-ride-(.*?)\/>/gi, '')

  private viewBox = (viewBox?: number[]): number[] => {
    if (viewBox) {
      this.schema.setAttribute('viewBox', viewBox.join(' '))
    }
  ...
  private fixSubstrates = () => {
    for (const station of this.stations) {
      ...
      const substrate = element.getElementsByClassName(
  ...
  private getStations = () =>
    Array.from(
      this.schema.getElementsByClassName(
