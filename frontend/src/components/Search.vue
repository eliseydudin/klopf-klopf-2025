...
const message = ref("")
const error = defineModel<string | null>("error", { required: true })
const fetchResult = defineModel<null | string>("result", { required: true })

const onClick = async () => {
  const { data, error: fetchError } = await (await useFetch(`http://0.0.0.0:8000/incident/station/statistics/${message.value}`)).text();
  if (fetchError.value) {
